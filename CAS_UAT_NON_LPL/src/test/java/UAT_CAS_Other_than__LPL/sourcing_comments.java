...
public class sourcing_comments extends BrowserSetup {
	...
	public void Sourcing(String sLogin_id,String sPassword,String sArea,String First_Name,String Last_Name,String Father_Name,String Product,
		...
		/*	logger = extent.startTest("Open Sourcing Screen");
			logger.assignAuthor("IT Testing Team");		
			logger.assignCategory("Regression_CAS_UAT");*/
			
			wait = new WebDriverWait(driver, 10);
				
//				fluentwait = new FluentWait(driver).withTimeout(6000, TimeUnit.MILLISECONDS).pollingEvery(2000, TimeUnit.MILLISECONDS);
		...
		/*	wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt("frameForwardToApp"));
			...
			if(Product.equalsIgnoreCase("TWO WHEELER LOAN")){
				
				selectproduct.selectByVisibleText("TWO WHEELER LOAN");		
				
			}else if(Product.equalsIgnoreCase("USED TWO WHEELER LOAN")){
				
				selectproduct.selectByVisibleText("USED TWO WHEELER LOAN");			
			}
			...
			if(Scheme.equalsIgnoreCase("IB - GENERAL")){
				
				schemes.sendKeys("IB - GENERAL");
				
			}else if(Scheme.equalsIgnoreCase("NIP - GENERAL")){
				
				schemes.sendKeys("NIP - GENERAL");
				
			}else if(Scheme.equalsIgnoreCase("NIP-USED TWL")){
				
				schemes.sendKeys("NIP-USED TWL");
			}				
			...
			assetmodel.sendKeys("Drum Kick Spoke");
	...
	/*	  @SuppressWarnings("rawtypes")
		public static void updateApplicationIdExcel() throws Exception {
		  ...
		  FileInputStream fis = new FileInputStream(ApplicationIdsfilepath); 
			...
			Row rowobj = Worksheet.getRow(rownumber);
			if (rowobj == null) {
